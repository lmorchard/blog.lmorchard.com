...
/**
 * The text of the first heading in a rendered post body, if any. Used both to
 * infer a post's title when it has no explicit one, and to avoid printing that
 * title twice in the published markdown.
 * @param {string} html - Rendered post body
 * @returns {string|undefined} Heading text, trimmed
 */
...
async function handleObsidianAttachments({ body: bodyIn, attributes }) {
  ...
  // Match Obsidian-style attachments: ![[image.jpg]]
  const obsidianRegex = /!\[\[(.*?)\]\]/g;
  let match;

  while ((match = obsidianRegex.exec(body)) !== null) {
    const imagePath = match[1];
    attachments.push(imagePath);
    // Replace the Obsidian image syntax with HTML
    // Keep the path as-is with spaces - browsers handle quoted attributes correctly
    body = body.replace(match[0], `<img src="${imagePath}" />`);
  }

  // Also match standard Markdown attachments: ![alt](attachments/image.jpg)
  // Only match relative paths (not starting with http:// or https://)
  const markdownRegex = /!\[.*?\]\((?!https?:\/\/)([^)]+)\)/g;

  while ((match = markdownRegex.exec(body)) !== null) {
    const imagePath = match[1];
    // Only add to attachments if not already included
    if (!attachments.includes(imagePath)) {
      attachments.push(imagePath);
    }
  }

  // Also match HTML img tags with relative src: <img src="attachments/image.jpg" />
  // Only match relative paths (not starting with http://, https://, or /)
  ...
  return { body, attributes: { ...attributes, attachments } };
}
