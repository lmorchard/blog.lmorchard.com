...
export async function loadPostFile(filename) {
  ...
  if (isMultipartPostFile(data)) {
    ...
    for (const part of parts) {
      if (!part) continue;
      const post = await parsePost(part, filename);
...
export async function parsePost(data, filename) {
  ...
  const slug = attributes.slug || slugFromPostName || slugify(title) || slugify(body);
...
// Extract first h1-h6 tag text as default title
