...
import { renderPostBody, extractTitleFromHTML } from "./postBody.js";
import { renderPostMarkdown } from "./postMarkdown.js";
...
/**
 * Set prev/next links to mean CHRONOLOGICAL order: prevPost* is the earlier
 * post, nextPost* the later one.
 *
 * sortPosts is descending (newest first), so the element BEFORE a post in the
 * array is chronologically later, not earlier. Walking the array and naming the
 * preceding element "prev" therefore inverted the meaning: prevPost* pointed at
 * the newer post and nextPost* at the older one, so the post header's left
 * arrow went forwards in time and its right arrow went backwards, and the
 * footer nav labelled the newer post "Previous".
 */
export async function linkPosts(posts) {
  // Clear before assigning. Posts loaded from the build cache arrive with
  // prevPost*/nextPost* already populated from a previous build, and the posts at
  // either END of the list get no assignment below — nothing is earlier than the
  // oldest or later than the newest. Without this reset a stale value survives,
  // which is how the newest post ended up with both arrows pointing at the same
  // place. (_cachedPrevPostPath/_cachedNextPostPath are captured at cache-load
  // time, before this runs, so the rebuild-invalidation check still works.)
  for (const post of posts) {
    post.prevPostPath = undefined;
    post.prevPostTitle = undefined;
    post.nextPostPath = undefined;
    post.nextPostTitle = undefined;
  }

  let laterPost = null; // the previous array element, i.e. the NEWER post
  for (const post of posts) {
    if (laterPost) {
      laterPost.prevPostPath = post.path;
      laterPost.prevPostTitle = post.title;
      post.nextPostPath = laterPost.path;
      post.nextPostTitle = laterPost.title;
    }
    laterPost = post;
  }
}
...
export async function parsePost(data, filename, postAttributes = {}) {
  ...
  // A frontmatter thumbnail may be a bare filename sitting alongside the post,
  // which is how localize-images rewrites them. Resolve it against the post's
  // URL just like an image referenced from the body.
  if (
    post.thumbnail &&
    !/^[a-z]+:\/\//i.test(post.thumbnail) &&
    !post.thumbnail.startsWith("/")
  )
    post.thumbnail = url.resolve(
      `${config.site.baseurl}/${postPath}/`,
      post.thumbnail
    );
...
export async function buildPost(post, options = {}) {
  ...
  await writeFiles(postPath, {
    "index.md": renderPostMarkdown(config.site, post),
...
async function copyDirPostAssets(parentPath, postPath, options = {}) {
  ...
  // buildPost generates index.md itself (source body plus frontmatter), so the
  // raw source copy must not clobber it. copyWithOptimization would normally
  // skip it as older than the generated file, but don't lean on mtime order.
  let filter = [
    "**/*",
    "!.copyfilter",
    "!index.md",
    "!index.markdown",
    ...config.postCopyFilters,
  ];
  try {
...
export const updateRelativeUrls = (html, baseUrl) => {
  if (!html || !baseUrl) return html;

  const $ = cheerio.load(html);
  const isRelative = (url) => url && !url.match(/^(https?:\/\/|\/\/|#)/);

  $('img').each((_, el) => {
    const src = $(el).attr('src');
    if (isRelative(src)) $(el).attr('src', `${baseUrl}/${src}`);
  });

  $('a').each((_, el) => {
    const href = $(el).attr('href');
    if (isRelative(href)) $(el).attr('href', `${baseUrl}/${href}`);
  });
