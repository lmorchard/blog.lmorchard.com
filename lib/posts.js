...
async function renderPostBody(body) {
  ...
  // TODO: parse with cheerio once and pass that through pipeline?
  result = await fixupImages(result);

  return result;
}

async function fixupImages(body) {
...
async function fixupBadLineEndingQuirks(body) {
  ...
  result = result.replace(/<!--more-->/g, "<!--more-->\n\n");
