...
const RE_POST_DATE = new RegExp(/(\d{4})-(\d{2})-(\d{2})/);
...
export async function loadPostFile(file) {
  ...
  if (!isMultipartPostFile(data)) {  
    if (!RE_POST_NAME.test(postName)) return [];
    const [, year, month, day, slug] = RE_POST_NAME.exec(postName);  
    return [ await parsePost(data, file, postName, isDir, year, month, day, slug) ];
  }

  const [, year, month, day] = RE_POST_DATE.exec(postName);

  const chunks = splitMultipartPostFile(data);

  const posts = [];
  for (let i = 0; i < chunks.length; i++) {
    const chunk = chunks[i];
    if (chunk) {
      const slugWords = chunk.toLowerCase().replace(/[^a-z0-9\s]/g, '').trim().split(/\s+/).slice(0, 7);
      const slug = slugWords.join('-');
      const post = await parsePost(chunk, file, postName, false, year, month, day, slug);
      if (!post) continue;
      posts.push(post);
    }
  }
...
export async function parsePost(data, filename, postName, isDir, year, month, day, slugDefault) {
  let { attributes, body } = frontmatter(data);
  body = await handleIncludes(filename, body);

  const slug = attributes.slug || slugDefault;
