...
export async function parsePost(data, filename, postAttributes = {}) {
  ...
  const time = attributes.time || "12:00:00Z";
  const date = moment(attributes.date || `${year}-${month}-${day}T${time}`);
...
function splitMultipartPostFile(data) {
  // Array to store all parts of the multipart post
  const parts = [];
  
  // Initialize first part with default attributes and empty content array
  let currentPart = [{ ...DEFAULT_MULTIPART_POST_ATTRIBUTES }, []];
  ...
  for (const line of lines) {
    // If not a separator line, add to current part's content
    if (!line.startsWith(POST_MULTIPART_SEPARATOR)) {
      currentPart[1].push(line);
      continue;
    }

    // Found a separator - extract JSON attributes if present
    // Format: 8<--- {"type": "entry", ...}
    const attributesText = line
      .substring(POST_MULTIPART_SEPARATOR.length)
      .trim();
    let attributes = { ...DEFAULT_MULTIPART_POST_ATTRIBUTES };

    // Try to parse JSON attributes, fallback to defaults if invalid
    if (attributesText) {
      try {
        attributes = {
          ...attributes,
        ...
        // Ignore parse errors, use default attributes
      }
    }

    // Start a new part with the parsed attributes
    currentPart = [attributes, []];
    parts.push(currentPart);
  }

  // Process the parts: join lines into text and filter empty parts
  return parts
