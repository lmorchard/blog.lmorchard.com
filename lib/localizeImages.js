...
import { optimizeImage, formatBytes } from "./imageUtils.js";
 ...
 * @param {boolean} options.optimize - If true, optimize images after downloading (default: true)
 */
export async function localizeImages(postGlobs = [`${config.postsPath}/**/*.{md,markdown}`], options = {}) {
  const { dryRun = false, optimize = true } = options;
  ...
  for (const file of files) {
    await localizeImagesInPost(file, { dryRun, optimize });
  }
}
 ...
 * @param {object} options - Options object
 * @param {boolean} options.dryRun - If true, don't actually download or modify files
 * @param {boolean} options.optimize - If true, optimize images after downloading
 */
async function localizeImagesInPost(filepath, options = {}) {
  ...
  // Convert standalone .md files to directory-based posts
  ...
  const markdownMatches = [...content.matchAll(EXTERNAL_IMAGE_RE)];
  for (const match of markdownMatches) {
    ...
    try {
      ...
      if (optimize) {
        const result = await optimizeImage(localPath, { verbose: false });
        const filename = path.basename(localPath);
        console.log(`  ✓ Downloaded & optimized: ${filename} (${formatBytes(result.originalSize)} → ${formatBytes(result.optimizedSize)}, ${result.savingsPercent}% savings)`);
      } else {
  ...
  const htmlMatches = [...content.matchAll(EXTERNAL_IMAGE_SRC_RE)];
