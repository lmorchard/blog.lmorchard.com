...
async function buildRootIndex(posts) {
  ...
  await writeFiles(root, {
    ...
    "index-excerpts.rss": templateRss({
      site: config.site,
      posts: await filterOmitFuturePosts(posts.slice(0, 15)),
      fullText: false,
    }),
...
/**
 * Rebuild only the indexes affected by changed posts — for fast watch-mode updates.
 * Rebuilds root index, and only the specific year/month/tag indexes provided.
 * Deliberately omits all.html, archives.html, and sitemaps (not needed for preview).
 * @param {Array} postsIn - Full post collection (all posts, for populating index pages)
 * @param {object} affected - Sets of affected index keys
 * @param {Set<string>} affected.years - Year strings e.g. "2026"
 * @param {Set<string>} affected.months - Year/month strings e.g. "2026/02"
 * @param {Set<string>} affected.tags - Tag strings
 * @param {object} options
 */
export async function buildSelectiveIndexes(postsIn, { years, months, tags }, { showDrafts = false } = {}) {
  const root = config.buildPath;
  const posts = await filterOmitDrafts(postsIn, { showDrafts });
  ...
  const builds = [buildRootIndex(posts)];

  for (const year of years) {
    const yearPosts = postsByYear[year];
    if (yearPosts?.length) {
      builds.push(buildIndex({
        basePath: path.join(root, year),
        title: year,
        indexLabel: "Year",
        template: templateIndexYear,
        posts: yearPosts.slice(0, 1000),
      }));
    }
  }
...
async function buildIndex({ basePath, title, tag, indexLabel, template, posts }) {
  ...
  return writeFiles(basePath, {
    ...
    "index.html": template({
      ...
      page: {
        ...
        tag,
        indexLabel,
        // Site-relative directory for this index, e.g. "2026", "2026/02",
        // "tag/foo" — used to link the feeds and index.json written alongside
        indexPath: path
          .relative(config.buildPath, basePath)
          .split(path.sep)
          .join("/"),
      },
    }),
