...
But, one thing I've learned is that friction adds up in user interfaces. It can be a squelch threshold that filters out noise what might be worth considering signal. Creating new files worked well enough when I was trying to hold back until I had Big Things to write. But, that was enough of a drag to keep me from posting much at all.

My usage of Obsidian as a private daybook got me thinking, though: I used to write that way in public using Dave Winer's OPML Editor. Why couldn't I do that with my own system now? Create one file every day and accumulate thoughts across the hours, occasionally allow larger notions bud off into fuller discrete posts as necessary.
